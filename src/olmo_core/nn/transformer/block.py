...
class MAGReorderedNormTransformerBlock(TransformerBlock):
    ...
    def inference_forward(self, x: torch.Tensor, *, loss_div_factor: Optional[Union[torch.Tensor, float]] = None, nan_audit: bool = False, **kwargs) -> torch.Tensor:
        ...
        # Always reset MLPs for each forward pass (like train_forward)
        self.memory.reset_mlps()
        
        # Process the sequence in chunks, just like train_forward
        gates = []  # list to store the gates
        for i in range(1 + ((x.size(1) - 1) // self.chunk_size)):
            ...
            try:
                # First retrieve from the chunk (matching train_forward's order)
                chunk_gate = self.memory.retrieve(chunk_x)
                gates.append(chunk_gate)
                
                # Then update with the chunk
                with torch.no_grad():  # Prevent backprop during inference
                    _loss = self.memory.update(chunk_x)
                
                if nan_audit:
                    print(f"[NAN AUDIT] Processed chunk {i}, gate shape: {chunk_gate.shape}")
            except Exception as e:
                if nan_audit:
                    print(f"[NAN AUDIT] Exception in chunk {i}: {str(e)}")
                raise
        
        # Concatenate all gates to form the full gate cache for this forward pass
        all_gates = torch.cat(gates, dim=1)
        
        # Handle attention with persistent tokens if needed
        if self.use_global_sw:
            ...
            try:
                attn = self.attention(x_with_persistent, **kwargs)[:, self.num_global_tokens:, :]
            except Exception as e:
                if nan_audit:
                    print(f"[NAN AUDIT] Exception in attention with persistent tokens: {str(e)}")
                raise
        else:
            try:
        ...
        try:
            # Apply the memory gate to attention
            sigmoid_result = nn.Sigmoid()(all_gates)
            ...
            if nan_audit:
                print(f"[NAN AUDIT] Exception in final computation: {str(e)}, gate shape: {all_gates.shape}, attn shape: {attn.shape}")
            raise
    ...
    def train_forward(self, x: torch.Tensor, *, loss_div_factor: Optional[Union[torch.Tensor, float]] = None, **kwargs) -> torch.Tensor:
        ...
        for i in range(1 + ((x.size(1) - 1) // self.chunk_size)):
            ...
            chunk_x = x[:, start:end, :]
            # Moved retrieve to before update to preserve causality
