...
def train_and_test(model, optimizer, train_loader, valid_loader, test_loader, n_epochs=20, print_loss=False):
    ...
    for epoch in tqdm(range(n_epochs)):
        ...
        for x, y in (train_loader):
            ...
            # # Debug prints for the failing iteration
            # print(f"Current x shape: {x.shape}")
            # print(f"Current y shape: {y.shape}")

            processed_x = x.float().to(device)
            model_output = model(processed_x)
            out = model_output[:,:-1].to(device)
            labels = y[:,1:].float().to(device)
            if out.shape[-1] == 1 and out.shape[-1] != labels.shape[-1]:
                out = out.squeeze(-1)

            # print(f"Shape of 'out' for loss: {out.shape}")
            # print(f"Shape of 'labels' for loss: {labels.shape}")
