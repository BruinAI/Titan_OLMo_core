...
class MACTitan(nn.Module):
    ...
    def forward(self, x):
        ...
        device = x.device
        res = torch.zeros((x.shape[0], x.shape[1], self.output_dim)).to(device)
        ...
        x = np.transpose(sliding_window_view(x.cpu(), self.context_window, axis=1), (0,1,3,2))
        residual = x.shape[1] % self.context_window

        stz = torch.from_numpy(x[:,:-residual].reshape(x.shape[0], -1, self.context_window, self.context_window, self.input_dim).copy()).to(device)
        ...
        residual_part = torch.from_numpy(x[:,-residual:].reshape(-1, self.context_window, self.input_dim)).to(device)
